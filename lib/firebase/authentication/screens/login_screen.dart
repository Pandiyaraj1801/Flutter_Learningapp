...
import 'package:learnapp/firebase/authentication/screens/register_screen.dart';
...
class LoginAuthFBScreen extends StatefulWidget {
  const LoginAuthFBScreen({super.key});

  @override
  State<LoginAuthFBScreen> createState() => _LoginAuthFBScreenState();
}

class _LoginAuthFBScreenState extends State<LoginAuthFBScreen> {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              submitBtn: () {
                if (_emailController.text.isEmpty ||
                  ...
                  AuthenticationServices.getAuthInstance
                      .logInUser(_emailController.text, _pwdController.text)
                      .then((val) {
                        showSnackBar(context, val, const Color(0xFF4CAF50));
                        Navigator.of(context).pushReplacement(
                          MaterialPageRoute(
                            builder: (context) => Scaffold(
                              body: Center(child: Text("Fuck Soceity")),
                            ),
                          ),
                        );
                      })
            ...
            SizedBox(height: 10),
            InkWell(
              child: Text("Register Account?", style: TextStyle(fontSize: 15)),
              onTap: () => Navigator.of(context).push(
                MaterialPageRoute(builder: (context) => RegisterAuthFBScreen()),
              ),
            ),
          ],
        ),
